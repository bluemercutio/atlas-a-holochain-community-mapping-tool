...
pub fn validate(op: Op) -> ExternResult<ValidateCallbackResult> {
    match op.to_type::<EntryTypes, LinkTypes>()? {
        OpType::StoreEntry(store_entry) => {
            match store_entry {
                OpEntry::CreateEntry { app_entry, action } => {
                    match app_entry {
                        EntryTypes::TagItem(tag_item) => {
                            validate_create_tag_item(
            ...
            match update_entry {
                OpUpdate::Entry {
                    ...
                    match (app_entry, original_app_entry) {
                        (
                            EntryTypes::TagItem(tag_item),
                            EntryTypes::TagItem(original_tag_item),
                        ) => {
                            validate_update_tag_item(
                                action,
                                tag_item,
                                original_action,
                                original_tag_item,
            ...
            match delete_entry {
                OpDelete::Entry { original_action, original_app_entry, action } => {
                    match original_app_entry {
                        EntryTypes::TagItem(tag_item) => {
                            validate_delete_tag_item(action, original_action, tag_item)
                        }
                    }
                }
                _ => Ok(ValidateCallbackResult::Valid),
        ...
        OpType::RegisterCreateLink {
            ...
            match link_type {
                LinkTypes::TagItemUpdates => {
                    validate_create_link_tag_item_updates(
                        action,
                        base_address,
                        target_address,
                        tag,
        ...
        OpType::RegisterDeleteLink {
            ...
            match link_type {
                LinkTypes::TagItemUpdates => {
                    validate_delete_link_tag_item_updates(
                        action,
                        original_action,
                        base_address,
                        target_address,
        ...
        OpType::StoreRecord(store_record) => {
            match store_record {
                ...
                OpRecord::UpdateEntry {
                    ...
                    match app_entry {
                        EntryTypes::TagItem(tag_item) => {
                            let result = validate_create_tag_item(
                                EntryCreationAction::Update(action.clone()),
                                tag_item.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_tag_item: Option<TagItem> = original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                                let original_tag_item = match original_tag_item {
